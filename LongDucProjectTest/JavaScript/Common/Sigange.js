...
////Function get data for Daily chart
//function SubmitDailyChart(ProjectName) {
//    timeselectDaily = document.getElementById('selecttimeDaily').value;
//    StartTimeDaily = timeselectDaily + stringStartHour;
//    EndTimeDaily = timeselectDaily + stringEndHour;
//    longPoliingEnergyDaily(ProjectName);
//}
...
function SubmitTenYearChart(ProjectName) {
    //timeselectYearly = document.getElementById('selecttimeTenYear').value;
    //var year = timeselectYearly.substring(0, 4);
    year = 2023;
    //for (i = 9; i > -1; i--) {
    //    recentYears[9-i] = (year - i).toString();
    //}
    for (i = 0; i < 10; i++) {
...
function updateChartYearly(data) {
    ...
    //var max = Math.ceil(Math.max(...yearlyself) / 100000) * 100000; // Giá trị tối đa (làm tròn lên đến hàng nghìn)
    //var tickInterval = max / 3; // Khoảng cách giữa các đường kẻ trên trục Y (chia thành 4 phần)
    ...
    Highcharts.chart('container_yearlyself', {
        ...
        yAxis: [{
            labels: {
                format: '{value}',
                formatter: function () {
                    return Math.round(this.value).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","); // Làm tròn, thêm dấu ',' sau giá trị số
                }
            },
            title: {
                useHTML: true,
            ...
            // Không cần thiết lập min, max, tickInterval, Highcharts sẽ tự động tính toán
        }],
        legend: {
...
function updateChartTenYear(data) {
    ...
    // Tính toán giá trị max và tickInterval dựa trên dữ liệu
    //var max = Math.ceil(Math.max(...yearlyself) / 1000000) * 1000000; // Giá trị tối đa (làm tròn lên đến hàng nghìn)
    //var tickInterval = max / 3; // Khoảng cách giữa các đường kẻ trên trục Y (chia thành 4 phần)
