...
3. **[OverviewRealtime.js](file:///c:/Users/tanhv/Project/WebApp_LongDuc_22012025Phase2/WebApp_LongDuc_22012025Phase2/LongDucProjectTest/JavaScript/RealTime/OverviewRealtime.js)**:
   ...
   - Cập nhật định dạng hiển thị của BOM sản xuất: Dữ liệu BOM được phân tách và hiển thị dưới dạng các bảng xếp chồng riêng biệt cho từng mẻ (ví dụ: "BOM sản xuất - Mẻ 1", "BOM sản xuất - Mẻ 2"). Cột "Mẻ" dư thừa ở trong bảng đã được loại bỏ, bảng chỉ còn hiển thị các cột: Mã hàng (`code`), Tên nguyên vật liệu (`material_code`), Tổng xuất (`quantity`), Đơn vị tính (`unit`), và Lot/No (`batch_no`).
